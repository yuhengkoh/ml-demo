...
I suspect that the normalization of the features is not being done correctly, leading to poor model performance.
...
for df in df_lst:
    df["z_norm"] = (df["fitness_scaled"] - df["fitness_scaled"].mean()) / df["fitness_scaled"].std()  # Normalize the fitness scores
